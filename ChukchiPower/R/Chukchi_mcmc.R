#' function to perform Bayesian analysis of single species count data for photograph-based analysis (no misID)
...
#' @param Thin  An (n.days X n.hours X n.iter) array providing n.iter posterior samples of the thinning parameters
...
Chukchi_mcmc<-function(formula,Data,Effort,spat.mod=0,Offset,Area.adjust,Control,Assoc=NULL,K=NULL,DayHour=NULL,Thin=NULL,Names.gam=NULL,Knots.gam=NULL,Prior.pars=NULL,Precision.pars=NULL){
  #require(mvtnorm)
  #do a little checking
  if(!(spat.mod %in% c(0,1,2)))cat('ERROR: spat.mod value not recognized (must be 0,1, or 2)')
  ...
  if(is.null(Control$predict))Control$predict=TRUE
  Count=Effort$Counts
  ...
  if(is.null(Thin))Thin=array(1,dim=c(1,1,1))
  if(is.null(DayHour))DayHour=matrix(1,n,2)

  #declare index, function for accessing thinning probabilities for specific transects
  Thin.pl=DayHour[,1]*(DayHour[,2]-1)+DayHour[,1]
  get_thin<-function(Thin,Thin.pl,iter){
    Tmp.thin=Thin[,,iter][Thin.pl]
    Tmp.thin
  } 
  thin.pl=sample(c(1:dim(Thin)[3]),1)
  ...
  Nu=rep(0,S)
  Nu[Mapping]=Count/(Offset*get_thin(Thin,Thin.pl,thin.pl))
  ...
  if(spat.mod==2){ #define some matrices, etc. needed for spatial updates under srr model
    ...
    if(Control$srr.tol>1){ #in this case, the number of eigenvalues is preselected
      ...
      if(max(Eigen$values)<Control$srr.tol)cat(paste("\n Error: maximum eigenvalue (",max(Eigen$values),") < Control$srr.tol; decrease srr.tol"))
  ...
  for(iiter in 1:Control$iter){
    ...
    Cur.thin=get_thin(Thin,Thin.pl,thin.pl)
    Mu=X.obs%*%Beta+Eta[Mapping]+Gamma[Mapping]
    sd=sqrt(1/tau.epsilon)
    full.cond.old=dnorm(Nu[Mapping],Mu,sd,log=1)+dpois(Count,Cur.thin*exp(Log.offset+Nu[Mapping]),log=1)
    Prop=Nu[Mapping]+runif(n,-Control$MH.nu,Control$MH.nu)
    full.cond.new=dnorm(Prop,Mu,sd,log=1)+dpois(Count,Cur.thin*exp(Log.offset+Prop),log=1)
    ...
    #update thin.pl - index into thinning array
    if(is.null(Thin)==FALSE){
      new.ind=round(runif(1,0.5,dim(Thin)[3]+0.5))
      Cur.thin=get_thin(Thin,Thin.pl,thin.pl)
      New.thin=get_thin(Thin,Thin.pl,new.ind)
      old.post=sum(dpois(Count,Cur.thin*exp(Log.offset+Nu[Mapping]),log=TRUE))
      new.post=sum(dpois(Count,New.thin*exp(Log.offset+Nu[Mapping]),log=TRUE))   
    ...
    if(iiter>Control$burnin & iiter%%Control$thin==0){
      ...
      Thin.mc[(iiter-Control$burnin)/Control$thin]=thin.pl
      if(is.null(Names.gam)==FALSE){
  ...
  Out=list(MCMC=list(Beta=Beta.mc,tau.epsilon=Tau.epsilon.mc,Pred=Pred.mc,Thin=Thin.mc),Accept=Accept,Control=Control)
  if(spat.mod>=1){
