#example simulation driver for Chukchi power analysis
#Actual analysis was split into multiple files and run simultaneously
#library(Matrix)
library(GenKern)  #to model sampling density
#library(hierarchicalDS)
library(ChukchiPower)
dir.create("./sim_data/", showWarnings = FALSE)
#load("./R/data/Grid_chukchi.Rda")
#load("./R/data/Thin.Rda")

data(Grid_chukchi)
data(Thin)
data(Sim_effort)
#load("Grid_chukchi_ExpDensities_PB.Rdata")
#load("Thin_samples.Rda")
...
#an example plot: expected polar bear densities across simulations
Cur.abund=matrix(Grid_chukchi$Grid[["D.pbear"]]*625,S,1)
Temp=vector("list",1)
plot_N_map(Cur.abund,Grid=Grid_chukchi$Grid)


if(SIM){
  for(isim in 1:n.sims){
    Sim.abund=sim_density_PB(Grid_chukchi)
...
Strata=as.factor(Grid_chukchi$Grid@data[,"D.bearded"])
Hab.cov=Grid_chukchi$Grid
Hab.cov@data$Strata=Strata
Hab.cov@data$Seals=Grid_chukchi$Grid[["D.ringed"]]+5*Grid_chukchi$Grid[["D.bearded"]]
Hab.cov@data$Seals=Hab.cov$Seals/mean(Hab.cov$Seals)
Hab.cov@data$sqrt.mainland=sqrt(Grid_chukchi$Grid[["dist_mainland"]])
...
adapt=TRUE
Area.hab=1-Grid_chukchi$Grid[["land_cover"]]
...
for(imod in 1:1){
  ...
  for(ifl in 2:2){
    ...
    for(isim in 1:n.sims){      
      ...
      XY=Grid_chukchi$Chukchi_centroids[Mapping,]
      X=XY$x-min(XY$x)
      Y=XY$y-min(XY$y)
      X=X/max(X)
      Y=Y/max(Y)
      ...
      Samp.dens=rep(0,S)
      XY=Grid_chukchi$Chukchi_centroids
      X=XY$x-min(XY$x)
      Y=XY$y-min(XY$y)
      X=X/max(X)
      Y=Y/max(Y)
