#master simulation driver for Chukchi power analysis - this one uses Bengtson strata as covariate
library(Matrix)
...
for(isp in 1:2)Hab.pois.formula[[isp]]=~Strata #dist_mainland+sqrt.mainland+Easting+Northing+EastNorth
...
for(isim in 1:n.sims){
  ...
  for(ifl in 2:4){
    ...
    save(Out,file=paste('./sim_data/Out2_Fl',ifl,'_sim',isim,'.Rda',sep=''))  
    #estimate abundance and save MCMC results
  }
}


#Cur.abund=matrix(apply(Out$Post$N[2,200:2000,],2,'mean'),S,1)
